...
color_scheme <- c("Too Little" = "deepskyblue", "Too Much" = "tomato", "About Right" = "goldenrod")
...
```
The conditions are met. By CLT, the confidence interval for a population proportion is [\hat{p} \pm z^\star \sqrt{\frac{\hat{p}(1-\hat{p})}{n}} ]
...
Let's extract the data that is relevant to question 1.3 to a new dataframe. Here we'll be performing our [\chi]-square test over data of all years in the sample, so the "year" variable is irrelevant:
...
```
We can see that the variable "degree" is factor type (categorical) with 5 levels, and there're 1010 missing values in the dataset represented by NA. We'll remove rows that contain missing values:
...
```{r}
# Create the two-way table with totals
two_way_table <- deg_educ %>%
  count(degree, nateduc) %>%
  pivot_wider(names_from = nateduc, values_from = n) %>%
  mutate(Total = `Too Little` + `Too Much` + `About Right`)  # Add row total for each degree

# Add column total for atheist and religious
two_way_table <- two_way_table %>%
  bind_rows(
    summarise(., degree = "Total", across(`Too Little`:Total, sum))
  )
```

```{r}
# transpose the table
transposed_table <- two_way_table %>%
  pivot_longer(cols = -degree, names_to = " ", values_to = "count") %>%
  pivot_wider(names_from = degree, values_from = count)
```


```{r}
# Display the table using `gt` for formatted output
transposed_table %>%
  gt() %>%
  tab_header(title = "Distribution of Opinion by Degree") %>%
